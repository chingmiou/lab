    ...
    public class JoeyJoinTests
    {
        ...
        public void all_pets_and_owner_get_pet_fullname()
        {
            ...
            var actual = JoeyJoin(employees, pets,
                (employee, pet) => $"{pet.Name}-{employee.LastName}", employee1 => employee1, pet1 => pet1.Owner);
        ...
        {
            var employeeEnumerator = outer.GetEnumerator();
            var petEnumberator = inner.GetEnumerator();
            while (employeeEnumerator.MoveNext())
            {
                var outerCurrent = employeeEnumerator.Current;
